...
### Client-level streaming (SDK core)

For application-level streaming without the agent runner:
...
# Async generator chunks
async for chunk in client.stream("Summarize in 10 words", model="gpt-4o-mini", max_tokens=64):
    print(chunk, end="")

# Awaitable wrapper with usage
resp = await client.stream_with_usage("Summarize in 10 words", model="gpt-4o-mini", max_tokens=64)
print(resp.get_text())
print(resp.usage)
```
